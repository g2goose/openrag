...
async def store_conversation_thread(
    user_id: str, response_id: str, conversation_state: dict
):
    ...
    try:
        ...
        async for chunk in response:
            ...
            tool_related_attrs = [
                ...
                or "call" in attr.lower()
                or "retrieval" in attr.lower()
            ]
                ...
                if isinstance(chunk_data, dict) and not detected_tool_call:
                    ...
                    has_results = any(
                        [
                            ...
                            and isinstance(chunk_data.get("results"), list),
                            "outputs" in chunk_data
                            and isinstance(chunk_data.get("outputs"), list),
                    ...
                    if has_results:
                        ...
                        detected_tool_call = (
                            True  # Mark that we've injected a tool call
                        )
...
async def delete_user_conversation(user_id: str, response_id: str) -> bool:
    ...
    try:
        ...
        if (
            user_id in active_conversations
            and response_id in active_conversations[user_id]
        ):
            del active_conversations[user_id][response_id]
            logger.debug(
                f"Deleted conversation {response_id} from memory for user {user_id}"
            )
            deleted = True

        # Delete from persistent storage
        conversation_deleted = (
            await conversation_persistence.delete_conversation_thread(
                user_id, response_id
            )
        )
        ...
        try:
            ...
            logger.debug(
                f"Released session ownership for {response_id} for user {user_id}"
            )
        except Exception as e:
        ...
        logger.error(
            f"Error deleting conversation {response_id} for user {user_id}: {e}"
        )
        return False
